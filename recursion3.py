...
def groupSumClump(start, nums, target):
	...
	count = start
	sum = 0
	while (count < len(nums)) and (nums[start] == nums[count]):
		sum += nums[start]
		count += 1
	if groupSumClump(count, nums, target - sum):
		return True
	if groupSumClump(count, nums, target):
		return True
	return False
...
def splitArray(nums):
	...
	sum1 = 0
	sum2 = 0
	return splitArrayHelper(nums,index,sum1,sum2)

def splitArrayHelper(nums,index,sum1,sum2):
	if index >= len(nums):
		return (sum1 == sum2)
	return (splitArrayHelper(nums,index + 1,sum1 + nums[index],sum2)) or (splitArrayHelper(nums,index + 1,sum1,sum2 + nums[index]))
...
def splitOdd10(nums):
	...
	sum1 = 0
	sum2 = 0
	return splitOdd10Helper(nums,index,sum1,sum2)

def splitOdd10Helper(nums,index,sum1,sum2):
	if index >= len(nums):
		return (sum1 % 10 == 0) and (sum2 % 2 != 0)
	return (splitOdd10Helper(nums,index + 1,sum1 + nums[index],sum2)) or (splitOdd10Helper(nums,index + 1,sum1,sum2 + nums[index]))
...
def split53(nums):
	...
	sum5 = 0
	sum3 = 0
	return split53Helper(nums,index,sum5,sum3)

def split53Helper(nums,index,sum5,sum3):
	if index >= len(nums):
		return (sum5 == sum3)
	if nums[index] % 5 == 0:
		return split53Helper(nums,index + 1,sum5 + nums[index],sum3)
	if nums[index] % 3 == 0:
		return split53Helper(nums,index + 1,sum5,sum3 + nums[index])
	return (split53Helper(nums,index + 1,sum5 + nums[index],sum3)) or (split53Helper(nums,index + 1,sum5,sum3 + nums[index]))
